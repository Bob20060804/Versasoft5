...
namespace Ersa.Platform.Plc.AnfoQuitt
{
	...
	public class EDC_AnfoQuittDienst : INF_AnfoQuittDienst
	{
		...
		public EDC_AnfoQuittDienst(Inf_CommunicationService i_edcKommunikationsDienst)
		{
		...
		public Task FUN_fdcAnfoSetzenUndQuittierenBehandelnAsync(EDC_ToggleWert i_edcToggle, CancellationToken i_fdcCancellationToken = default(CancellationToken))
		{
			...
			{
				await FUN_fdcTestUndInitAnfoWertAsync(i_edcToggle, i_fdcCancellationToken).ConfigureAwait(true);
				SUB_BooleanParameterWertSchreiben(i_edcToggle.PRO_edcToggle,true);
		...
		{
            // 读
			m_edcKommunikationsDienst.Sub_ReadValue(i_edcToggleWert.PRO_edcToggle);
		...
		private async Task<bool> FUN_fdcAnfoWertZurueckSetzenAsync(EDC_BooleanParameter i_edcAnfoParameter, CancellationToken i_fdcCancellationToken)
		{
			...
			do
			{
				...
				m_edcKommunikationsDienst.Sub_ReadValue(i_edcAnfoParameter);
				i32AnzahlVersuche++;
			}
